 ...
 * @internal
 */
#[CoversClass(RetransmissionManager::class)]
final class RetransmissionManagerTest extends TestCase
{
    ...
    /**
     * @param array<array{int, int}> $ackRanges
     */
    ...
    public function testOnAckReceivedWithOutOfOrderAck(): void
    {
        // 发送包1-10
        for ($i = 1; $i <= 10; ++$i) {
            $packet = $this->createMockPacket();
    ...
    public function testOnPtoTimeoutBasicFunctionality(): void
    {
        // 发送一些包但不确认
        for ($i = 1; $i <= 3; ++$i) {
            $packet = $this->createMockPacket();
    ...
    public function testOnPtoTimeoutNoUnackedPackets(): void
    {
        $result = $this->retransmissionManager->onPtoTimeout(2000.0);
    ...
    public function testCalculateRetransmissionDelayMaxRetransmissions(): void
    {
        // 测试最大重传次数的延迟计算
    ...
    public function testShouldFastRetransmitInitialState(): void
    {
        $this->assertFalse($this->retransmissionManager->shouldFastRetransmit());
    }

    public function testShouldFastRetransmitWithDuplicateAcks(): void
    {
        // 发送包
        for ($i = 1; $i <= 10; ++$i) {
            $packet = $this->createMockPacket();
    ...
    public function testGetRetransmissionStatsAfterActivity(): void
    {
        // 发送包并模拟重传活动
        for ($i = 1; $i <= 5; ++$i) {
            $packet = $this->createMockPacket();
    ...
    public function testIsInRetransmissionStormInitialState(): void
    {
        $this->assertFalse($this->retransmissionManager->isInRetransmissionStorm());
    }

    public function testIsInRetransmissionStormAfterHighRetransmissionRate(): void
    {
        // 模拟高重传率场景
        for ($i = 1; $i <= 20; ++$i) {
            $packet = $this->createMockPacket();
    ...
    public function testEdgeCases(): void
    {
        ...
        // 验证重传统计没有发生异常变化
        $statsAfter = $this->retransmissionManager->getRetransmissionStats();
        $this->assertEquals($stats['total_retransmissions'], $statsAfter['total_retransmissions']);
    }
}
